CREATE PROCEDURE [EDW].[USP_POPULATE_DATE_DIMENSION]
...
AS
	...
	--https://www.codeproject.com/Articles/647950/Create-and-Populate-Date-Dimension-for-Data-Wareho   (<-- not a typo)

	John Wild, 2018-10-02

	*/

	DELETE FROM EDW.DIM_DATE

	DROP TABLE IF EXISTS [dbo].[dimDate]

	SET DATEFIRST 1  -- <-- UK format, Monday is first day of week
...
--		, @CurrentDate DATETIME
	...
	***************************************************************************/
	...
	SET @dtFiscalYearStart = 'April 01, 1995'
	SET @FiscalYear = 1995
	SET @LastYear = 2052
	...
	***************************************************************************/
	...
	/*SET Fiscal Quarter*/
		...
		INSERT INTO @tb (PeriodDate, FiscalDayOfYear, FiscalWeekOfYear, FiscalMonth, FiscalQuarter, FiscalQuarterName, FiscalYear) VALUES 
		(@CurrentDate, @FiscalDayOfYear, @FiscalWeekOfYear, @FiscalMonth, @FiscalQuarter, @FiscalQuarterName, @FiscalYear)
	...
	UPDATE @tb
	SET
		FiscalMMYYY = RIGHT('0' + CONVERT(VARCHAR, FiscalMonth),2) + CONVERT(VARCHAR, FiscalYear)
	...
	FROM @tb a
		INNER JOIN [dbo].[DimDate]  ON a.PeriodDate = dbo.DimDate.[Date]
	...
	SELECT
		d.DateKey,
		d.[date],
		d.[DayOfWeekUSA], -- <- Take the US field, has already been adjusted for UK using SET DATEFIRST 1
		d.[DayOfMonth],
		d.[DayOfYear],
		d.[DayName],
		d.[WeekOfMonth],
		...
		d.[Quarter],
		d.[QuarterName],
		CASE WHEN d.[Quarter] IN (1,2) THEN 'First' WHEN d.[Quarter] IN (3,4) THEN 'Second' END,
		d.[Year],
		d.IsHolidayUK,
		~ d.IsWeekday,
		d.FiscalMonth,
		...
		d.FiscalQuarter,
		d.FiscalQuarterName,
		CASE WHEN d.FiscalQuarter IN (1,2) THEN 'First' WHEN d.[FiscalQuarter] IN (3,4) THEN 'Second' END,
		d.FiscalYear
	FROM
		DimDate d
